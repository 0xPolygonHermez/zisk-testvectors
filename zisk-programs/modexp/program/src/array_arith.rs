...
use ziskos::array_lib::{
    add_agtb, add_short, div_long, div_short, double, halve, mul_long, mul_short, U256,
};
...
pub fn array_arith_tests() {
    ...
    let res = double(&a);
    assert_eq!(res.len(), 1);
    assert_eq!(res, [U256::TWO]);

    // [4]
    let a = [U256::from_u64(4)];
    let res = double(&a);
    assert_eq!(res.len(), 1);
    assert_eq!(res, [U256::from_u64(8)]);

    // [4n, 4n]
    let a = [U256::from_u64(4), U256::from_u64(4)];
    let res = double(&a);
    assert_eq!(res.len(), 2);
    assert_eq!(res, [U256::from_u64(8), U256::from_u64(8)]);

    // [2^255, 2^255]
    let a = [U256_MAX_HALF, U256_MAX_HALF];
    let res = double(&a);
    assert_eq!(res.len(), 3);
    assert_eq!(res, [U256::ZERO, U256::ONE, U256::ONE]);
    ...
    let res = halve(&a);
    assert_eq!(res.len(), 2);
    assert_eq!(res, [U256::TWO, U256::TWO]);

    // [4n] * 10
    let a = vec![U256::from_u64(4); 10];
    let res = halve(&a);
    assert_eq!(res.len(), 10);
    assert_eq!(res, vec![U256::TWO; 10]);
    // ============================
}
